...
function show_copy($gotit = FALSE) {
    ...
    if ($gotit == 'none') {
        $found = true;
        echo 'The administrator doesn\'t want to show you that this is Copyright 2008 <a href"http://www.eticketsupport.com">eTicket Support</a> so we will be quiet and honor this request';
        ...
        <tr>
        <td><a href="http://www.eticketsupport.com" target="_blank">Powered by eTicket</a></td>
        ';
...
function check_update($check_freq) {
    global $db_table;
    $urltoupdate = 'http://www.eticketsupport.com/version.php?version=' . LANG_VERSION;
        if (function_exists('curl_init')) {
            $update = curl_init();
            curl_setopt($update, CURLOPT_RETURNTRANSFER, TRUE);
            curl_setopt($update, CURLOPT_URL, $urltoupdate);
            curl_setopt($update, CURLOPT_USERAGENT ,"eTicket (LANG_VERSION)");
            $contents = curl_exec($update);
            curl_close($update);
        } else {
            $contents = file_get_contents($urltoupdate);
        }
