...
if ($type == "upgrade") {
    $admin_info = @mysql_fetch_array(mysql_query("SELECT password FROM " . $_POST[db_table_prefix] . "reps WHERE username = 'admin'"));
    if (md5($_POST["admin_pass"]) != $admin_info["password"]) {
...
if (!isset($errors)) {
    ...
    $file = '..' . DIRECTORY_SEPARATOR . 'pipe.php';
    ...
    $file = '..' . DIRECTORY_SEPARATOR . 'settings.php';
    $str = file_get_contents($file);
    if ($handle = fopen($file, 'w')) {
...
if ($type == "install") {
    ...
    $queries = array();
    $queries[] = "INSERT INTO `" . $db_table_prefix . "reps` VALUES (1, '" . $_POST[admin_name] . "', '" . $_POST[admin_email] . "', 'admin', '" . md5($_POST[admin_pass]) . "', '', '1')";
    $queries[] = "INSERT INTO `" . $db_table_prefix . "categories` VALUES (1, '" . $_POST[cat_name] . "', '', '', '', '" . $_POST[cat_email] . "', '', 0, 'url');";
...
if (isset($_POST['enable_ftp']) && !isset($errors)) {
    ...
    <li>Visit <a href="http://www.eticketsupport.com">eTicket</a>.</li>
    </ul>';
} else {
    $output = '<p><a href="../">eTicket</a> appears to have installed successfully!</p>
    <p>Things to do next:</p>
